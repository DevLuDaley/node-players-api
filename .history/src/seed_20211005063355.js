...
const transformApiData = async (apiResponse) => {
  try {
    ...
    const playersWithAges = apiResponse.filter((player) => player.age != null);
    console.log('apiResponse.length', apiResponse.length);
    console.log('playersWithAges.length', playersWithAges.length);
    console.log('playersWithAges[0]', playersWithAges[0]);
